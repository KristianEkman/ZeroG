...
typedef enum { PERSPECTIVE_STM, PERSPECTIVE_WHITE } ScorePerspective;
...
TrainingSample *load_epd_file(const char *path, int *out_count,
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (res == 0) {
      ...
      if (count >= capacity) {
        capacity *= 2;
        TrainingSample *new_samples =
            realloc(samples, capacity * sizeof(TrainingSample));
        if (!new_samples) {
          fprintf(stderr,
                  "Error: Out of memory reallocating samples for '%s'.\n",
                  path);
          free(samples);
          fclose(f);
          return NULL;
...
void print_help(const char *prog_name) {
  ...
  printf("Options:\n");
  printf("  -i, --input <file>     Input EPD file (default: "
         "quiet_training_positions_evaluated.epd)\n");
  printf("  -o, --output <file>    Output weights file (default: "
         "nn_weights.bin)\n");
  printf("  -w, --weights <file>   Initial weights file to continue training "
         "from (optional)\n");
  printf("  -e, --epochs <num>     Number of training epochs (default: 30)\n");
  printf("  -b, --batch-size <num> Batch size for training (default: 4096)\n");
  printf(
      "  -t, --threads <num>    Number of worker threads (default: 16 max)\n");
  printf("  -l, --lr <value>       Initial learning rate (default: 0.001)\n");
  printf("  -v, --val-split <val>  Validation split ratio (default: 0.1)\n");
  printf("  --val-file <file>      Static validation EPD file (disables "
         "validation split if provided)\n");
  printf("  -d, --wd <value>       Weight decay coefficient (default: 1e-4)\n");
  printf("  -p, --score-perspective <persp> Score perspective: 'stm' or "
         "'white' (default: stm)\n");
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-i") == 0 || strcmp(argv[i], "--input") == 0) {
      ...
      if (i + 1 < argc)
        weights_path = argv[++i];
    } else if (strcmp(argv[i], "--val-file") == 0 ||
               strcmp(argv[i], "--validation") == 0) {
      if (i + 1 < argc)
        val_file_path = argv[++i];
      ...
      if (i + 1 < argc)
        initial_lr = (float)atof(argv[++i]);
    } else if (strcmp(argv[i], "-v") == 0 ||
               strcmp(argv[i], "--val-split") == 0) {
      if (i + 1 < argc)
        val_split = (float)atof(argv[++i]);
    } else if (strcmp(argv[i], "-d") == 0 || strcmp(argv[i], "--wd") == 0) {
      if (i + 1 < argc)
        weight_decay = (float)atof(argv[++i]);
    } else if (strcmp(argv[i], "-p") == 0 ||
               strcmp(argv[i], "--score-perspective") == 0) {
      if (i + 1 < argc) {
        ...
        if (strcmp(persp, "stm") == 0) {
          ...
          fprintf(stderr,
                  "Invalid score perspective '%s'. Use 'stm' or 'white'.\n",
                  persp);
          return 1;
        }
      }
  ...
  if (epochs <= 0 || batch_size <= 0 || num_threads <= 0 ||
      !isfinite(initial_lr) || initial_lr <= 0.0f || !isfinite(val_split) ||
      val_split <= 0.0f || val_split >= 1.0f || !isfinite(weight_decay) ||
      weight_decay < 0.0f) {
  ...
  if (val_file_path) {
    ...
    global_val_set =
        load_epd_file(val_file_path, &global_val_size, perspective);
    if (!global_val_set || global_val_size == 0) {
      fprintf(stderr,
              "Error: Failed to load static validation dataset from '%s'\n",
              val_file_path);
  ...
  printf("\nStarting training (%d epochs per file, initial lr = %.5f, wd = "
         "%.1e)...\n",
  ...
  for (int file_idx = 0; file_idx < num_files; file_idx++) {
    ...
    TrainingSample *dataset =
        load_epd_file(cur_file, &parsed_count, perspective);
    if (!dataset) {
      fprintf(stderr,
              "Warning: Could not read or parse EPD file '%s'. Skipping.\n",
              cur_file);
      continue;
    }
    ...
    if (global_val_set) {
      ...
      if (train_size < 1 || val_size < 1) {
        fprintf(stderr, "Dataset is too small for the selected validation "
                        "split. Skipping.\n");
        free(dataset);
        continue;
      }
    ...
    for (int epoch = 1; epoch <= epochs; epoch++) {
      ...
      for (int i = 0; i < train_size; i += batch_size) {
        ...
        float b_loss =
            nn_train_batch_parallel(batch_trainer, nn, &train_set[i],
                                    cur_batch_size, current_lr, weight_decay);
        if (!isfinite(b_loss)) {
          fprintf(stderr,
                  "Error: Training failed at file %d, epoch %d, sample %d\n",
                  file_idx + 1, epoch, i);
          free(dataset);
          if (global_val_set)
            free(global_val_set);
      ...
      float avg_val_loss = nn_evaluate_batch_parallel(
          batch_trainer, nn, val_set, val_size, &avg_quant_val_loss);
      ...
      printf(
          "Epoch %2d (Global %2d) | Train Loss: %.6f | Float Val Loss: %.6f | "
          "Quant Val Loss: %.6f | LR: %.6f | Time: %.2fs (%.1fk pos/s)%s\n",
          epoch, global_epoch, avg_train_loss, avg_val_loss, avg_quant_val_loss,
          current_lr, epoch_time, kpos_per_sec, is_best ? " *" : "");
