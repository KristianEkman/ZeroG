...
static int pvs(Position *pos, int depth, int ply, int alpha, int beta,
...
/* Lazy/incremental move selection: picks the best move from the remaining
 * unsorted range */
static inline void pick_best_move(Move *moves, int *scores, int count,
                                  int current_idx) {
...
static inline void check_time_limit(uint64_t start_time,
                                    const SearchLimits *limits) {
  node_count++;
...
/* Checks if a castling move is illegal (king under attack or passing through
 * attack) */
...
static int try_null_move_pruning(Position *pos, int depth, int ply, int beta,
                                 uint64_t start_time,
                                 const SearchLimits *limits,
...
static int search_aspiration_window(Position *pos, unsigned depth,
...
static int reconstruct_pv_from_tt(const Position *pos, PVLine *pv,
                                  int max_depth) {
...
static void log_search_info(unsigned depth, int score, uint64_t nodes,
...
int search_best_move_with_limits(const Position *board,
  ...
  for (unsigned d = 1; d <= max_search_depth; d++) {
    ...
    // Reconstruct the PV from the transposition table to get a complete PV
    // trace
    PVLine tt_pv;
